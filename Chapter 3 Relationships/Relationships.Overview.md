...
#### Use Technology
1. Create a new page: `ctrl + doc▾ (+page)` then `4  Add Lists & Spreadsheet`.
2. Fill out the first two columns with data.
3. Name these columns: `tab`, `enter` and enter a name, then `▶︎` ,  `▲` and `enter` to put another name in.
4. Create a new page for plots: `ctrl + doc▾ (+page)` then `5  Add Data & Statistics`.
5. Click on the bottom and the left of the screen and choose the x-variable as well as the y-variable named in 3.
6. Make a ***regression***: `menu › 4  Analyze › 6 Regression › 1  Show Linear (mx+b)`
7. Check the ***correlation r***: `ctrl + doc▾ (+page) › 1  Add Calculator` to create a calculator page, then click `var` and find `stat.r`.
8. Check the residuals: find `stat.Resid` in `var`.